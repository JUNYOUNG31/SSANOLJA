<template>
  <div class="telestation-container paper-input" style="color: black">
    <div
        ...
        <div style="display: flex" v-if="index % 2 === 0">
          ...
          <div v-if="index !== 0">
            ...
            <button @click="worstPick(index)">
              <v-icon color="red">{{
                index === worst - 1 ? "mdi-thumb-up" : "mdi-thumb-up-outline"
              }}</v-icon>
            </button>
          </div>
        </div>
    ...
    >
      ...
      <button v-show="isRoomMaker" @click="sendMessageToEveryBody('', 'room')">
        다시하기
      </button>
    </div>
  </div>
...
<script
  type="module"
  src="https://unpkg.com/wired-elements/lib/wired-progress.js?module"
></script>
<script
  type="module"
